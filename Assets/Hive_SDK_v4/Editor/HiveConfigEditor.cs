...
namespace Hive.Unity.Editor
{
    ...
    public class HiveConfigEditor : UnityEditor.Editor
    {
        ...
        private bool showGoogleSignIn = true;//HiveConfig.UseGooglePlay;

        private bool showGooglePlayGames = true;
        ...
        private static GUIContent googleDeviceFlowClientIdLabel = new GUIContent("Google Device Flow Client Id [?]:","This is the Client ID for using Google Device Flow. For more details, please refer to https://developers.google.com/identity/protocols/oauth2/limited-input-device");
...
//        private static GUIContent HIVEiOSAppIdLabel = new GUIContent("HIVE App Id [iOS]:");
        ...
        private static GUIContent unsupportedUaShareTypeLabel = new GUIContent("unsupportedUaShareType [?]","Promotion UA items not supported (SMS, URL, KAKAO, LINE, WHATSAPP, FACEBOOK, MESSENGER, WECHAT");
        private static GUIContent valueLabel = new GUIContent("Value: ");
        ...
        //Advance/CrashReport
        private static GUIContent saveAnalyticsLogBeforeCrashLabel = new GUIContent("saveAnalyticsLogBeforeCrash [?]","iOS 전용 기능으로. 앱이 NSException 타입 에러 또는 Unix singal 타입 에러 때문에 강제 종료되기 직전 상황을 감지하여 서버에 전송하지 않고 대기중인 Hive 애널리틱스 로그를 파일로 저장합니다. 외부 서드 파티 크래시 리포터를 이용할 경우 충돌 방지를 위해 false로 설정하여 비활성화 해야 합니다. 4.16.3 미만에서 사용되었던 useCrashReport 값은 saveAnalyticsLogBeforeCrash 값으로 사용됩니다. (+v4.16.3)");
        ...
        public class UnsupportedUAShareType
        {
            ...
            public virtual void OnGUI(HiveConfigEditor editor)
            {
                if(values != null && values.Count > 0)
                {
                    ...
                    {
                        EditorGUILayout.BeginHorizontal();
                        valuesArr[i] = editor.EditableTextField(valueLabel, valuesArr[i]);
        ...
        public override void OnInspectorGUI()
        {
            ...
            // 내부 스크롤 영역 시작 
            scrollPos = EditorGUILayout.BeginScrollView(scrollPos, GUILayout.Height(UnityEngine.Screen.height / 3));
            ...
            {
                EditorGUILayout.Separator();
                this.ExternalDependencyUtilGUI();
            }
        ...
        private void AppIdGUI()
        {
            ...
            if (SelectedPlatform == HiveConfigXML.Android)
            {
                ...
                EditorGUI.indentLevel++;
                this.showGoogleSignIn = EditorGUILayout.Foldout(this.showGoogleSignIn, "Google SignIn Settings" + SelectedPlatformPostfix);
                ...
                // Google Play Games in Android
                EditorGUI.indentLevel++;
                this.showGooglePlayGames = EditorGUILayout.Foldout(this.showGooglePlayGames, "Google Play Games Settings" + SelectedPlatformPostfix);
                if (this.showGooglePlayGames)
                {
                    EditorGUILayout.BeginHorizontal();
            ...
            } else if (SelectedPlatform == HiveConfigXML.macOS || SelectedPlatform == HiveConfigXML.Windows)
            {
                ...
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.BeginHorizontal();
                SelectedPlatform.googleDeviceFlowClientID = EditableTextField(googleDeviceFlowClientIdLabel, SelectedPlatform.googleDeviceFlowClientID);
                EditorGUILayout.EndHorizontal();
            }
        ...
        private void HiveConfigGUI()
        {
            ...
            if (this.showHiveInitSettings)
            {
                ...
                if (SelectedPlatform == HiveConfigXML.Android)
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(unsupportedUaShareTypeLabel, fieldName);
                    EditorGUILayout.EndHorizontal();

                    UnsupportedUAShareType unsupportedUAShareType = SelectedPlatform.unsupportedUAShareType;
                    if (unsupportedUAShareType != null)
                        unsupportedUAShareType.OnGUI(this);
                    SelectedPlatform.unsupportedUAShareType = unsupportedUAShareType;
                }
            }
            EditorGUI.indentLevel--;
            ...
            if (this.showHiveAdvanceSettings)
            {
                ...
                if (SelectedPlatform == HiveConfigXML.iOS) {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(saveAnalyticsLogBeforeCrashLabel, fieldName);
                    SelectedPlatform.saveAnalyticsLogBeforeCrash = EditorGUILayout.Toggle( SelectedPlatform.saveAnalyticsLogBeforeCrash, both);
                    EditorGUILayout.EndHorizontal();
                }
            }
            EditorGUI.indentLevel--;
        ...
        private void IOSUtilGUI()
        {
            ...
            // if (this.showIOSSettings)
            {
                ...
                if (GUILayout.Button("Build iOS project post process settings"))
                {
                    HivePostProcessMenu.showHivePostProcessEditorIOS();
                }
            }
        ...
        private void ExternalDependencyUtilGUI()
        {
            if (GUILayout.Button("ExternalDependency settings (for Android, iOS)"))
            {
        ...
        private void AndroidUtilGUI()
        {
            ...
            // if (this.showAndroidUtils)
            {
                ...
                if (PlayerSettings.Android.applicationEntry == AndroidApplicationEntry.Activity)
                    ...
                    this.ReadOnlyTextField(classNameLabel, HiveManifestMod.HiveUnityMainGameActivity);
            #else
                this.ReadOnlyTextField(classNameLabel, HiveManifestMod.HiveUnityMainActivity);
            #endif
        ...
        private string EditableTextField(GUIContent label, string value)
        {
            string returnValue;
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(label, GUILayout.MaxWidth(190), GUILayout.MinWidth(30), GUILayout.Height(16));
        ...
        void DrawUILine(Color color, int thickness = 1, int padding = 40)
        {
