    ...
    public static class HivePostProcess
    {
        ...
        private static void iOSSettingPodFile(string buildPath) {
            ...
            using (StreamWriter sw = File.AppendText(buildPath + "/Podfile"))
            {
                ...
                sw.WriteLine("      end");
                sw.WriteLine("      xcconfig_path = config.base_configuration_reference.real_path");    // Xcode 15.x Toolchain Issue
                sw.WriteLine("      xcconfig = File.read(xcconfig_path)");
                sw.WriteLine("      xcconfig_mod = xcconfig.gsub(/DT_TOOLCHAIN_DIR/, 'TOOLCHAIN_DIR')");
                sw.WriteLine("      File.open(xcconfig_path, 'w') { |file| file << xcconfig_mod }");
